USE [antasenaDBTest]
GO
/****** Object:  StoredProcedure [dbo].[tp_deleteTipePeriode]    Script Date: 7/13/2025 5:44:30 PM ******/
SET ANSI_NULLS ON
GO
...
ALTER procedure [dbo].[tp_deleteTipePeriode]
	@PeriodId int,
	@by varchar(max)
as
...
if @PeriodeInf = 0 begin
	...
	BEGIN
		...
		if @ecode = 0 Begin
			...
			select TipePeriodeId,
				...
				JumlahHariKoreksi,
				TipeHarian,
				UseKoreksi, @by upduser, getdate() upddate, 'D' updflag
			from dbo.MSTipePeriodeHarian
			where PeriodName=@PeriodName
			set @ecode = @ecode + @@ERROR
		end
		if @ecode = 0 Begin
			DELETE FROM [dbo].[MSTipePeriodeHarian]
			WHERE PeriodName=@PeriodName
			set @ecode = @ecode + @@ERROR
		end
	END
	...
	BEGIN
		...
		if @ecode = 0 Begin
			DELETE FROM [dbo].[MSTipePeriodeMingguan]
			WHERE PeriodName=@PeriodName
			set @ecode = @ecode + @@ERROR
		end
	END
	...
	BEGIN
		...
		if @ecode = 0 Begin
			DELETE FROM [dbo].[MSTipePeriodeBulanan]
			WHERE PeriodName=@PeriodName
			set @ecode = @ecode + @@ERROR
		end
	END
	...
	BEGIN
		...
		if @ecode = 0 Begin
			DELETE FROM [dbo].[MSTipePeriodeTriwulan]
			WHERE PeriodName=@PeriodName
			set @ecode = @ecode + @@ERROR
		end
	END
	...
	BEGIN
		...
		if @ecode = 0 Begin
			DELETE FROM [dbo].[MSTipePeriodeSemester]
			WHERE PeriodName=@PeriodName
			set @ecode = @ecode + @@ERROR
		end
	END
	...
	BEGIN
		...
		if @ecode = 0 Begin
			DELETE FROM [dbo].[MSTipePeriodeTahun]
			WHERE PeriodName=@PeriodName
			set @ecode = @ecode + @@ERROR
		end
	END
	...
	if @ecode = 0 Begin
		...
		select PeriodId, PeriodName, PeriodType, FgMove, getdate() upddate, @by upduser, 'D' updflag
		from dbo.MSTipePeriode
		where PeriodId = @PeriodId
		set @ecode = @ecode + @@ERROR
	end
