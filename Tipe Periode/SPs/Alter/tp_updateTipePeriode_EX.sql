...
GO
	ALTER procedure [dbo].[tp_updateTipePeriode_EX] @PeriodId int,
...
,
	...
	@by varchar(max) as --DECLARE @PeriodName_ varchar(max)=(select PeriodName from dbo.MSTipePeriode WHERE PeriodId = @PeriodId)
	Declare @ecode int Begin Tran
set
	@ecode = @ @ERROR
select
	@PeriodName_Ori = PeriodNameOri
from
	[MSTipePeriodeHarianExc]
WHERE
	PeriodName = @PeriodName IF @PeriodType = 'Harian' BEGIN if @ecode = 0 Begin
UPDATE
...
END IF @PeriodType = 'Mingguan' BEGIN if @ecode = 0 Begin
UPDATE
...
END IF @PeriodType = 'Bulanan' BEGIN if @ecode = 0 Begin
UPDATE
...
END IF @PeriodType = 'Triwulanan' BEGIN if @ecode = 0 Begin
UPDATE
...
END IF @PeriodType = 'Semesteran' BEGIN if @ecode = 0 Begin
UPDATE
...
END IF @PeriodType = 'Tahunan' BEGIN if @ecode = 0 Begin
UPDATE
