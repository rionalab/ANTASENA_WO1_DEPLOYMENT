...
ALTER FUNCTION [dbo].[F_GetTrussteeInformation] (
    @UserName VARCHAR(100)
)
RETURNS @listInformasi TABLE (
...
AS
BEGIN
   SET @UserName = dbo.fn_StripCharacters(@UserName, '^a-zA-Z0-9 [-]_@.!#$&/\[]');

   DECLARE @existsInformation VARCHAR(1);

   IF NOT EXISTS (
        SELECT 1
   FROM MSUser WITH (NOLOCK)
   WHERE UserName = @UserName AND FgActive = 'Y'
    )
        RETURN;

   IF EXISTS (
        SELECT 1
   FROM MSUserInformation WITH (NOLOCK)
   WHERE UserName = @UserName
    )
        SET @existsInformation = '1';
    ELSE
        SET @existsInformation = '0';

   IF (@existsInformation = '0')
    BEGIN
      INSERT INTO @listInformasi
         (
         idinformation, namainformasi, kelompokinformasi,
         periode, periodestr, parentinformation, parentnamainformasi
         )
      SELECT DISTINCT
         b.idinformation,
         b.namainformasi,
      ...
      FROM PeriodeInformation a WITH (NOLOCK)
   ...
   IF (@existsInformation = '1')
    BEGIN
      ...
      SELECT
         ...
         b.parentinformation,
         b.parentnamainformasi
      FROM MSUserInformation a WITH (NOLOCK)
         INNER JOIN v_allinformation b WITH (NOLOCK)
         ON a.IDInformasi = b.idinformation
