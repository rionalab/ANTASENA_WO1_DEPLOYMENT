...
GO
	...
	ALTER procedure [dbo].[pg_getTreeInformasi] @flag varchar(1) = '',
	@kelompokinformasi varchar(100) = '',
	@PeriodTypeId varchar(100) = '',
	@GroupId varchar(100) = '0' as If OBJECT_ID('tempdb.dbo.#temp_pg_kelinf') is not null begin DROP TABLE #temp_pg_kelinf
end
select
	distinct a.kelompokinformasi,
	a.idinformation,
	d.PeriodTypeId into #temp_pg_kelinf
from
	...
	inner join MSGroupInformation as g on a.idinformation = g.IDInformasi
	and d.PeriodTypeId = g.Periode
where
	g.GroupId = @GroupId if @flag = '0' begin declare @pilihan varchar(1),
	@GidUser int
select
...
select
	...
	case
		when b.kelompokinformasi is not null then 1
		else 0
	end as [check],
	@pilihan as pilihan,
	case
		when @GidUser > 0 then 1
		else 0
	end as GidUser
from
	metadata_mgmt as a
...
select
	...
	d.PeriodTypeId,
	case
		when v.idinformation is not null then 1
		else 0
	end as [check]
from
	v_allinformation as a
