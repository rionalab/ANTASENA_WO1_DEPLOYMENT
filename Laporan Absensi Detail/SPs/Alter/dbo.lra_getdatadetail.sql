...
ALTER Procedure [dbo].[lra_getdatadetail]
   --Declare        
...
--set @tblhnamelra = 'ZZLRA_aGVyeXVzbUBnbWFpbC5jb20='        
...
set @tblhnamelra =  dbo.fn_StripCharacters(@tblhnamelra, '^a-zA-Z0-9 [-]_@.!#$&/\[]')                                    
set @tblnamefileinformasi =  dbo.fn_StripCharacters(@tblnamefileinformasi, '^a-zA-Z0-9 [-]_@.!#$&/\[]')                                    
set @idpelapor =  dbo.fn_StripCharacters(@idpelapor, '^a-zA-Z0-9 [-]_@.!#$&/\[]')                
        
set @Status=''''+REPLACE(@Status,',',''',''')+''''               
        
  If OBJECT_ID('tempdb..#temptableprocess') is not null                                                 
begin
   DROP TABLE #temptableprocess
end               
        
        
                                      
Create Table #temptableprocess
(
...
If OBJECT_ID('tempdb..#temptableprocess2') is not null                                                                                
begin
   DROP TABLE #temptableprocess2
end                                     
                                                    
                                         
                              
Create Table #temptableprocess2
(
   idpelapor varchar(10) null,
   versioncode varchar(10) null,
   Nama varchar(255) null,
...
If OBJECT_ID('tempdb..#temptablefileinfo') is not null                                     
begin
   DROP TABLE #temptablefileinfo
end                                     
                
        
        
                                      
Create Table #temptablefileinfo
(
   idpelapor varchar(10) null,
   versioncode varchar(10) null,
   versioncodealias varchar(10) null,
...
If OBJECT_ID('tempdb..#tabledetail') is not null                                                                                
begin
...
insert into #temptableprocess
exec(@q)                              
        
                              
set @q  = ' declare @bbb table (idpelapor           varchar(10)  null,                              
      versioncode         varchar(10)  null,                               
      versioncodealias    varchar(10)  null,           
      PeriodeData         varchar(10)  null,                              
      periodelaporan      varchar(30) null,                
...
select ROW_NUMBER() over (order by nama, sequence_information, recordtimestamp ) ,
   nama, kelompokinformasi, namainformasialias, versioncode, periodelaporanstr, periodedata, recordtimestamp, [text()]
from ( select distinct #temptablefileinfo.idpelapor+' - '+msinstansi.nama nama,
 ...
 convert(varchar(10), convert(datetime,replace(nullif(recordtimestamp,'null'),'T', ' ')),108)  AS [text()]
   from #temptablefileinfo
      ...
      inner join ( select distinct versioncode, isnull(sequence_information,999) sequence_information
      from v_allinformation ) metadata
      on metadata.versioncode = #temptablefileinfo.versioncodealias
   where exists ( select 1
   from #temptableprocess
...
update #tabledetail set jumlahbarislaporan =  a.rownumber        
from #tabledetail inner join
   ( select versioncode, recordtimestamp, PeriodeData, sum(  #temptablefileinfo.rownumber   ) rownumber
   from
      #temptablefileinfo
   where  #temptablefileinfo.recordtimestamp = #temptablefileinfo.laporantimestamp
   group by  versioncode, recordtimestamp, PeriodeData
...
update #tabledetail set JumlahBarisKoreksi = a.rownumber        
from #tabledetail inner join
   ( select versioncode, recordtimestamp, PeriodeData, sum(  #temptablefileinfo.rownumber   ) rownumber
   from
      #temptablefileinfo
   where  #temptablefileinfo.recordtimestamp = #temptablefileinfo.koreksitimestamp
   group by  versioncode, recordtimestamp, PeriodeData
) a on #tabledetail.versioncode = a.versioncode
      and #tabledetail.recordtimestampori = a.recordtimestamp
      and #tabledetail.periodedata = a.PeriodeData        
...
--update #tabledetail set jumlahbarislaporan = #temptablefileinfo.rownumber        
...
select seq, Bank, #tabledetail.kelompokinformasi, #tabledetail.namainformasi, #tabledetail.versioncode,
   periodelaporan, periodedata, recordtimestamp , convert(varchar, Format(convert(DECIMAL, JumlahBarisLaporan), '#,0'))  JumlahBarisLaporan,
   convert(varchar,Format(convert(DECIMAL, JumlahBarisKoreksi),'#,0')) JumlahBarisKoreksi
from #tabledetail
