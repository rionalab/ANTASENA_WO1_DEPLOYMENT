...
GO
ALTER procedure [dbo].[usr_getTreeInformasi]
   --'0','','','616',''    
   --declare  
   @flag varchar(1)='',
...
select @fgOpt=FgPelaporOpt
from msgroup
where GroupId=@groupid    
    
declare @pilihan varchar(1)     
...
CREATE TABLE dbo.#temp_pg_kelinf
(
...
If OBJECT_ID('tempdb.dbo.#temp_usr_kelinf') is not null                                             
 begin
   DROP TABLE #temp_usr_kelinf
end     
    
 select distinct a.kelompokinformasi, a.idinformation, d.PeriodTypeId
into #temp_usr_kelinf
from v_allinformation as a
   inner join
   (select b.idinformation    
     , case when c.PeriodType='Harian' then 'D'    
   ...
   inner join MsUserInformation as g on a.idinformation=g.IDInformasi and d.PeriodTypeId=g.Periode
where g.UserName=@UserName    
  
  
If OBJECT_ID('tempdb.dbo.#tempinformasi') is not null                                             
 begin
   DROP TABLE #tempinformasi
end     
  
 select distinct kelompokinformasi
into #tempinformasi
from v_allinformation  
...
if @flag='0'    
 ...
 Begin
     ...
     , case when u.kelompokinformasi is not null then 1 else 0 end as [check]    
     , @pilihan as pilihan
      from #tempinformasi as a
  ...
  begin
      if exists(select FgInformationOpt
      from MSUser
      where UserName=@UserName)    
    begin
         select @pilihan= FgInformationOpt
 ...
 begin
   select distinct a.kelompokinformasi+'|'+d.PeriodTypeId as [id]    
    ...
    , d.PeriodTypeId    
    , case when u.idinformation is not null then 1 else 0     
    end as [check]
   from v_allinformation as a
      inner join
      (select b.idinformation    
 ...
 begin
   select distinct a.kelompokinformasi, a.idinformation+'|'+d.PeriodTypeId as [id] , a.namainformasi [desc]    
    , case when (select count(1)
      from MsUserInformation
      where UserName=@UserName and IDInformasi=a.idinformation and Periode=d.PeriodTypeId)    
