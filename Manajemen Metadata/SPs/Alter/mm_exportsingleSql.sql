...
ALTER procedure [dbo].[mm_exportsingleSql]
	--'tkk01','tes'
...
declare @idinformation varchar(max), @return varchar(max)

set @idinformation = ( select top 1
	idinformation
from [metadata_mgmt]
...
declare @icount int=(select count(1)
...
set @return ='';
set @return=@return+'CREATE TABLE [dbo].'+@idinformation+'( '

select @return=@return+
idelement+' '
				+case when idtodata='string' then 'varchar('+convert(varchar,[length]) +'), '
...
FROM [dbo].[metadata_mgmt]
WHERE [versioncode] = @versioncode and idtodata is not null
ORDER BY [sequence] asc
set @return=@return+'PRIMARY KEY(';
select @return=@return+ 
	case when compositekey_yn = 'y' then [idelement]+',' end
FROM [dbo].[metadata_mgmt]
WHERE 
	...
	or
	(@startDate is not null and datestart >= @startDate)
	)
	and (
