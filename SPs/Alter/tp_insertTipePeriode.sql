USE [antasenaDBTest]
GO
/****** Object:  StoredProcedure [dbo].[tp_insertTipePeriode]    Script Date: 7/13/2025 4:37:41 PM ******/
SET ANSI_NULLS ON
GO
...
ALTER procedure [dbo].[tp_insertTipePeriode]
...
Declare @ecode int,@@id int,@isexists int=0
...
if @ecode = 0 and @isexists=0 Begin
	...
	select PeriodId, PeriodName, PeriodType, FgMove, getdate(), @by, 'I'
	from dbo.MSTipePeriode
	where PeriodId=@@id
...
IF @PeriodType='Harian' AND NOT EXISTS(SELECT *
	FROM dbo.MSTipePeriodeHarian
	WHERE PeriodName=@PeriodName)
	BEGIN

	if @ecode = 0 and @isexists=0 Begin
		INSERT INTO dbo.MSTipePeriodeHarian
			(PeriodName
			,[JamBukaPelaporan]
			,[JamBukaKoreksi]
	...
	if @ecode = 0 and @isexists=0 Begin
		...
		select TipePeriodeId,
			...
			JumlahHariKoreksi,
			TipeHarian,
			UseKoreksi, @by upduser, getdate() upddate, 'I' updflag
		from dbo.MSTipePeriodeHarian
...
IF @PeriodType='Mingguan' AND NOT EXISTS(SELECT *
	FROM dbo.MSTipePeriodeMingguan
	WHERE PeriodName=@PeriodName)
	BEGIN
	if @ecode = 0 and @isexists=0 Begin
		INSERT INTO dbo.MSTipePeriodeMingguan
			(PeriodName
...
IF @PeriodType='Bulanan' AND NOT EXISTS(SELECT *
	FROM dbo.MSTipePeriodeBulanan
	WHERE PeriodName=@PeriodName)
	BEGIN
	if @ecode = 0 and @isexists=0 Begin
		INSERT INTO dbo.MSTipePeriodeBulanan
			(PeriodName
			,[MulaiBatasPelaporan]
			,[AkhirBatasPelaporan]
			,[BatasKeterlambatan])
...
IF @PeriodType='Triwulanan' AND NOT EXISTS(SELECT *
	FROM dbo.MSTipePeriodeTriwulan
	WHERE PeriodName=@PeriodName)
	BEGIN
	if @ecode = 0 and @isexists=0 Begin
		INSERT INTO dbo.MSTipePeriodeTriwulan
			(PeriodName
...
IF @PeriodType='Semesteran' AND NOT EXISTS(SELECT *
	FROM dbo.MSTipePeriodeSemester
	WHERE PeriodName=@PeriodName)
	BEGIN
	if @ecode = 0 and @isexists=0 Begin
		INSERT INTO dbo.MSTipePeriodeSemester
			(PeriodName
...
IF @PeriodType='Tahunan' AND NOT EXISTS(SELECT *
	FROM dbo.MSTipePeriodeTahun
	WHERE PeriodName=@PeriodName)
	BEGIN
	if @ecode = 0 and @isexists=0 Begin
		INSERT INTO dbo.MSTipePeriodeTahun
			(PeriodName
			,MulaiBatasPelaporan
			,AkhirBatasPelaporan
			,BatasKeterlambatan)
