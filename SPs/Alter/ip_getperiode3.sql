...
BEGIN
   IF (@periode = 'D')  
    BEGIN
      IF @status = 'KP'  
        BEGIN
         SELECT selected, test, Label, b.kelompokinformasi
         FROM (  
                SELECT
               CASE WHEN d.idinformation IS NULL THEN 0 ELSE 1 END AS selected,
               v.idinformation,
               ...
               v.namainformasi +  
                        ...
                        END AS Label
            FROM (  
                    SELECT DISTINCT idinformation, namainformasi, parentinformation
               FROM metadata_mgmt  
                ) v
               INNER JOIN v_cakupanabsensi c ON v.idinformation = c.idinformation
               LEFT JOIN (  
                    SELECT DISTINCT idinformation
            ...
            GROUP BY   
                    v.idinformation, v.namainformasi, c.tipeperiod, d.idinformation  
            ) a
            JOIN (  
                SELECT DISTINCT idinformation, kelompokinformasi
            FROM metadata_mgmt  
            ) b ON a.idinformation = b.idinformation
         ORDER BY kelompokinformasi, a.idinformation
      END  
        ELSE  
        BEGIN
         SELECT selected, test, Label, b.kelompokinformasi
         FROM (  
            ...
            FROM (  
               ...
               WHERE   
                        LEFT(periode, 1) = @periode
                  AND LEFT(a.IdPelapor, 3) = LEFT(@idpelapor, 3)
                  AND (b.statuskantor = 'KC' OR b.statuskantor = 'KPO')
                  AND b.jeniskegiatanoperasionalbank = @jenis  
                ) d ON c.idinformation = d.idinformation
            WHERE   
                    c.tipeperiod IN ('D','D1','D2','D3','D4','D5','D6')
